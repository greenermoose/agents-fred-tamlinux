...
Panel {
  ...
  // Token-less agents (Cursor, Antigravity) carry tokensAvailable: false and
  // render prompt counts instead of token charts.
  readonly property bool promptMode: provider !== null && provider.tokensAvailable === false
  ...
  function dayTooltip(day, today) {
    ...
    // Prompt-mode agents (no local token ledger) show raw prompt counts;
    // sessions only exist for today, so they ride along there instead of
    // taking a section of their own.
    if (root.promptMode) {
      var promptText = label + " · " + count + " prompts"
      if (today) promptText += " · " + Number(provider.todaySessions || 0) + " sessions"
      return promptText
    }
    var text = label + " · " + usage.formatTokenCount(count) + " tokens"
      ...
      Flickable {
        ...
        Column {
          ...
          Column {
            ...
            PanelSectionHeader {
              width: parent.width
              text: root.promptMode ? "PROMPTS BY DAY" : "TOKENS BY DAY"
              foreground: root.foreground
              fontFamily: root.fontFamily
            }
          ...
          Column {
            id: modelSection
            // Prompt-mode agents never model tokens; modelUsage is empty so
            // this hides on its own, and the guard keeps it off even if a
            // prompt-mode record ever carried stray usage.
            visible: !root.promptMode && root.models.length > 0
            width: parent.width
            spacing: Style.spacing.md
  ...
  component DayRow: Item {
    ...
    Text {
      id: dayValue
      textFormat: Text.PlainText
      // Prompt-mode agents render raw counts: formatTokenCount's K/M/B
      // shorthand would crush a "1.5K prompts" that is really just 1523.
