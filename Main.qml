...
Item {
  ...
  // pick up a stock /usr/bin copy) and the collectors run under a closed
  ...
  readonly property var updateEnv: ({
    "PATH": "/usr/bin:/bin",
    "HOME": home,
    "TZ": Quickshell.env("TZ") || "",
    "LANG": Quickshell.env("LANG") || "C",
    "LC_ALL": Quickshell.env("LC_ALL") || "C",
    ...
    "CLAUDE_CONFIG_DIR": Quickshell.env("CLAUDE_CONFIG_DIR") || "",
    "CODEX_HOME": Quickshell.env("CODEX_HOME") || ""
  })
  ...
  function updateCommand(kind, agentIds) {
    var command = [root.updateHelperPath]
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      // Agents with no local token ledger (Cursor, Antigravity) say so; the
      // missing field on older snapshots means token mode.
      tokensAvailable: synced ? (stats.tokensAvailable !== false) : (record.tokensAvailable !== false),
  ...
  // replicas of the same upstream truth on every synced device, so the
  // widest value wins — summing them would double every token per machine.
  ...
  function aggregateSnapshots(snapshots) {
    ...
    for (var i = 0; i < snapshots.length; i++) {
      ...
      for (var providerId in snapshotProviders) {
        ...
        // A single token-less snapshot makes the whole merged provider
        // prompt-mode: the ledger either exists on the device or it doesn't.
        acc.tokensAvailable = acc.tokensAvailable !== false && stats.tokensAvailable !== false
    ...
    for (var id in providers) {
      ...
      outProviders[id] = {
        ...
        tokensAvailable: acc.tokensAvailable,
  ...
  function providerSnapshot(record) {
    return {
      ...
      tokensAvailable: record.tokensAvailable !== false,
